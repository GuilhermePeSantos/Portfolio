...
def aumentar(valor=0.0, porcent=0.0, formatar=False):
    ...
    return result if not formatar else moeda(result)  # 1 Forma de solução do return


def diminuir(valor=0.0, porcent=0.0, formatar=False):
    porcentagem = valor * (porcent / 100)
    result = valor - porcentagem
    return result if formatar is False else moeda(result)  # 2 Forma de solução do return


def dobro(valor=0.0, formatar=False):
    result = valor * 2
    if formatar:  # 3 Forma de solução do return
        return moeda(result)
    else:
        return result
